...
module.exports = (Store) => {
  class SequelizeSessionStore extends Store {
    constructor(options) {
      if (!options) {
        throw new SequelizeSessionStoreException(
          "Options with valid sequelize instance required."
        );
      }
      if (!options.db) {
        throw new SequelizeSessionStoreException(
          "No sequelize instance passed in."
        );
      }
      ...
      this.Session = require(path.join(__dirname, "models", "Session"))(
        options.db
      );
      this.Session.sync();
